...
namespace basis
{
    TPBasisComplex::TPBasisComplex( const std::shared_ptr<const param::TPParametricAtlas>& pa,
    ...
    ParentBasis TPBasisComplex::parentBasis( const topology::Cell& c ) const
    {
        if( c.dim() != mAtlas->cmap().dim() )
            throw std::runtime_error( "parentBasis only accepts cells of dimension cmap.dim()" );

        const auto [source_dart, line_dart, tp_op] = mAtlas->cmap().unflatten( c.dart() );
        const ParentBasis source_basis = mSourceComplex->parentBasis(
            topology::Cell( source_dart, mSourceComplex->parametricAtlas().cmap().dim() ) );
