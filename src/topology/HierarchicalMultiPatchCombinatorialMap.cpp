                            ...
                            const std::vector<std::vector<Cell>>& leaf_elements )
{
    ...
    // FIXME: Factor this out to use the same code as in HierarchicalTPCombinatorialMap
    for( size_t level = 0; level < refinement_levels.size() - 1; level++ )
    {
        for( const auto& leaf_elem : leaf_elements.at( level ) )
        {
            ...
            const auto phi_chain = [&level,&refinement_level,&mutable_constituents]( const size_t patch_ii, const topology::Face& leaf_face ){
                ...
                if( not all_darts_are_leaves )
                {
                    ...
                    do
                    {
                        ...
                        mutable_constituent.iterateLeafDescendants( d, [&]( const Dart& descendant ) {
                            ...
                            {
                                const auto [desc_level, desc_level_d] = mutable_constituent.dartRanges().toLocalDart( descendant );
                                const Dart phi1 = topology::phi( *mutable_constituent.refinementLevels().at( desc_level ), 1, desc_level_d ).value();
                                if( mutable_constituent.isLeaf( mutable_constituent.dartRanges().toGlobalDart( desc_level, phi1 ).id() ) )
                                {
                    ...
                    if( perform_phi1_chain )
                    {
                        ...
                        {
                            mutable_constituent.setPhi( key, value );
                        }
                    }
