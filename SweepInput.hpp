...
SweepInput refinedCube( const Eigen::Vector3i& n_refinements )
{
    // 5 tet cube
    ...
    const auto flatten_vert = [&n_refinements]( const Eigen::Vector3i& vert ) {
        return vert(0) + ( n_refinements( 0 ) + 1 ) * vert( 1 ) + ( n_refinements( 0 ) + 1 ) * ( n_refinements( 1 ) + 1 ) * vert( 2 );
    };
    ...
    for( int k = 0; k < n_refinements(2); k++ )
    {
        for( int j = 0; j < n_refinements(1); j++ )
        {
            for( int i = 0; i < n_refinements(0); i++ )
            {
                ...
                const std::array<int, 9 > local_verts = { 
                    flatten_vert( {i, j, k} ), 
                ...
                for( const Simplex& simp : ( ( k % 2 == 0 ) ? simplices_template_1 : simplices_template_1 ) )
                    simplices.push_back( { 
                        VertexId( local_verts.at( simp.vertex( 0 ).id() ) ), 
...
SweepInput bentRefinedCube( const Eigen::Vector3i& n_refinements )
{
    using namespace std::numbers;
    const SweepInput cube = refinedCube( n_refinements );

    std::vector<Eigen::Vector3d> new_points;
    std::transform( cube.points.begin(), cube.points.end(), std::back_inserter( new_points ),
        [&]( const Eigen::Vector3d& pt ){
            return Eigen::Vector3d( 0.1*pt(0), (1+0.1*pt(1))*sin(pi*pt(2)/2), (1+0.1*pt(1))*cos(pi*pt(2)/2) );
        } );
